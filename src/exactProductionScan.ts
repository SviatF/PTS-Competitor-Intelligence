...
const workspace = {
  id: 'bad8e12a-14b7-40b6-b27b-bfa99db7c96a',
  chatId: -1004411533743,
  projectName: 'PTS Cooperation',
  geo: 'EU',
};

const competitors = [
  {
    id: '9ae92498-889d-4839-a641-3cdb6050bc82',
    name: 'ADS Wind Agency',
    pageId: '229968643535520',
  },
  {
    id: '41516d3c-b8b8-4d73-b107-65306eab9dac',
    name: 'Traffic.Money.Agency',
    pageId: '961705790368723',
  },
];
...
async function extractAds(page: Page, expectedAdvertiser: string, pageId: string): Promise<ParsedAd[]> {
  const cards = await page.evaluate(() => {
    ...
    for (const marker of leaves) {
      ...
      for (let d = 0; d < 14 && node; d++) {
        ...
        if (text.length > 120 && /Sponsored/i.test(text) && /Started running on/i.test(text)) card = node;
  ...
  return cards.flatMap((card: any) => {
    const advertiser = parseAdvertiser(card.text);
    if (advertiser.toLowerCase() !== expectedAdvertiser.toLowerCase()) return [];
        ...
        for (const event of events) {
          if (event.type === 'STOPPED') {
            await notifier.sendStopped({
              chatId: workspace.chatId, projectName: workspace.projectName, competitorName: competitor.name,
              libraryId: String(event.ad.libraryId || ''), startedAt: event.ad.startedAt, lastSeenAt: event.ad.lastSeenAt,
            });
          } else {
            await notifier.sendNewAd({
              chatId: workspace.chatId, projectName: workspace.projectName, geo: workspace.geo,
              competitorName: competitor.name, eventType: event.type, ad: collected(event.ad),
            });
          }
        }
      } finally {
